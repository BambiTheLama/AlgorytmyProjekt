...
void SimplexNoise::SetParams(float frequency, float amplitude, float lacunarity, float persistence)
{
...
void SimplexNoise::RandPermArray()
{
    srand((unsigned)time(NULL));
    for (int i = 0; i < 256; i++) {
        uint8_t randomValue = 0 + (rand() % 256);

        perm[i] = randomValue;
    }
}

uint8_t* SimplexNoise::GetPermTable()
{
    return perm;
}
